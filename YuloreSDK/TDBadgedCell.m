...
- (void) layoutSubviews
{
    ...
    // Calculate the size of the bage from the badge string
		...
		CGRect badgeframe = CGRectMake(self.contentView.frame.size.width - (badgeSize.width + 13 + self.badgeRightOffset),10 +
    ...
    for (UILabel *label in self.resizeableLabels)
    {
      ...
      {
        //CGFloat textLabelWidth = badgeframe.origin.x - label.frame.origin.x - self.badgeLeftOffset + 20;
